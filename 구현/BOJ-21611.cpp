...
// ↑, ↓, ←, →
/**
 * 중대실수 1 : 나선형 회전을 할 때는 마지막 부분에 6번 하나 삐죽 티어나오도록 수식계산으로 풀고있었다.
 *      >> 그래서 for (int i = 0 ; i < n ; i++) 를 했었는데 여기서 문제가 발생했다. <-- 실제로 n==7 이면 7번 회전하기 때문.
 *      --> 이런 류가 나오면 cout 으로 좌표를 찍어보는 것도 도움이 된다. (삑사리가 날 수 있음.)
